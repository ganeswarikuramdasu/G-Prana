...
## Objective
- Fix all four doctor-module bugs in the PranaCrux Phase 6 Java migration (Spring Boot 3.3.4 backend + React 19/Vite frontend):
...
## Important Details
- Repo root `C:\Users\ganes\Downloads\pranacrux-java-migration-phase6` (git repo). Monorepo: `backend-java/` (Java 17, Spring Boot 3.3.4, MySQL, package `com.pranacrux` — NOT `com.nxushealth`) + `frontend/` (React 19, Vite, TypeScript, Tailwind). Backend on :8080, dev server on :5173.
...
## Work State
### Completed
- 2026-09-16 REBRAND: platform rebranded **G-Prana → PranaCrux** repo-wide. Java package `com.gprana` → `com.pranacrux` (dir moved, main class `GpranaApplication.java` → `PranacruxApplication.java`); artifact/jar `gprana-backend` → `pranacrux-backend` (`finalName` + pom `artifactId` + Dockerfile + jar name); DB default `gprana` → `pranacrux`; config prefix `gprana:` → `pranacrux:` (application.yml line 68) + `com.gprana` → `com.pranacrux` logger line; localStorage keys `gprana_*` → `pranacrux_*`; fallback emails `@gprana.org` → `@pranacrux.org`; brand strings in all .tsx/UI → "PranaCrux" (incl. "PranaCrux AI"); super-admin default password `Admin@Gprana2026!` → `Admin@Pranacrux2026!` (application.yml:79, DemoAccountsSeeder, LoginPage.tsx:134); V2 migration `SET @proc_schema = 'pranacrux'`; `pranacruxHealthId` query param in Hospital Controller/Service; logo file `public/g-prana-icon.svg` → `public/pranacrux-icon.svg` (same original NexusHealth heartbeat-pulse icon; refs in index.html/AppShell/LandingPage updated); GitHub Actions/workflow + README/AGENTS/.env.example/render.yaml updated. Deployment config now points at NEW infra names: `pranacrux-backend.onrender.com` (render.yaml APP_URL + vercel.json proxy destination + name) and Vercel `pranacrux-eight.vercel.app` (CORS_ORIGINS) + repo `https://github.com/ganeswarikuramdasu/PranaCrux.git`. CAUTION: those URLs will 404 until Render/Vercel services and the GitHub repo are renamed on the web dashboards (pattern from prior rebrands); the CURRENTLY-LIVE services are still `gprana-backend.onrender.com` and `g-prana-eight.vercel.app` (with `G-Prana.git` remote) — FLIP the config back to the live URLs if the user has not renamed the cloud services yet. Verified: `mvn -q -DskipTests package` + `npm run lint` + `npm run build` all clean; fresh jar `backend-java/target/pranacrux-backend.jar`. Remaining `@nexus.in` demo-account emails kept (real DB records / login credentials).
### Completed
...
## Next Move
- None — all four doctor-module bugs are fixed, verified, committed (`48ffd02`) and pushed to `https://github.com/ganeswarikuramdasu/PranaCrux.git`.
- Remaining note for the user: the persistent 500s were environmental (stale DB schema / old backend build). To apply the fix: `git pull`, stop the old app, drop/recreate the local `pranacrux` DB (or let `ddl-auto: update` rebuild it), and restart the backend from the latest build.

## Relevant Files
- `backend-java/src/main/java/com/pranacrux/service/DoctorService.java` — core of tasks 1–3 (`accessPatientRecords`, `createAccessSession`, card/consent logic).
- `backend-java/src/main/java/com/pranacrux/service/EmergencyService.java` — task 4 (`identify`, start-session/break-glass, records).
- `backend-java/src/main/java/com/pranacrux/service/RecordAccessLogService.java` — swallowed-exception log save (500 candidate); possibly needs REQUIRES_NEW.
- `backend-java/src/main/java/com/pranacrux/service/AuditLogService.java` (line 37) — best-effort log (same tx-poison risk if outer @Transactional).
- `backend-java/src/main/java/com/pranacrux/service/PatientResolver.java` — patient resolution by id / health id / email (null returns must be handled).
- `backend-java/src/main/java/com/pranacrux/repository/AccessCardRepository.java` — `findAllByAnyIdentifier` must be restricted to card identifiers for task 3.
- `backend-java/src/main/java/com/pranacrux/entity/RecordAccessLog.java` / `AuditLog.java` / `AccessSession.java` — fragile mirrored-FK entity pattern.
- `backend-java/src/main/java/com/pranacrux/common/GlobalExceptionHandler.java` — source of the generic 500 message; `ApiException` handler is correct path.
...
- `backend-java/src/main/java/com/pranacrux/repository/{AppointmentRepository,MedicalRecordRepository,ConsentRepository}.java` — NULL-tolerant queries used by the flows.
...
- Verified: 20-test API suite on a live MySQL pranacrux DB - granted flows returns 200, denied -> 403 clean, unknown patient -> 404, bad/missing input -> 400, card flow 403 for non-card tokens, no backend stack traces. Backend mvn -DskipTests package exit 0; frontend 
pm run lint + 
pm run build clean.
...
- Both dev servers currently running: backend jar target/pranacrux-backend.jar on 8080; frontend vite on 127.0.0.1:5173 (IPv4 only; vite's ::1-only binding is unreachable on this box - launch with `npx vite --host 127.0.0.1 --port 5173 --strictPort`).
- 2026-09-12 (commit 13d66cc) - all-method access sweep + 3 more 500/409 fixes:
  ...
  - LOCAL DB creds: root / `Ganeswari@2006`, db `pranacrux` (127.0.0.1:3306). Launch backend with env baked into `cmd /c "set MYSQL_HOST=127.0.0.1 && set MYSQL_PASSWORD=Ganeswari@2006 && ... java -jar ..."` (PowerShell `$env:` + Start-Process did NOT propagate on the 2026-09-12 relaunch, silently connecting to Aiven instead).

## Current Live State (2026-09-16)
- REBRAND G-Prana -> PranaCrux applied repo-wide (see Work State / 2026-09-16 REBRAND note above). Commits a45c1f6 (bulk rebrand, 150 files) + 9a2b926 (last NEXUSHEALTH->PRANACRUX tokens, QR prefixes) both pushed.
- Verified: backend `mvn -q -DskipTests package` -> `backend-java/target/pranacrux-backend.jar`; frontend `npm run lint` (tsc --noEmit) + `npm run build` (vite) clean.
- Zero old-brand tokens remain in code (only AGENTS.md history notes + the intentional legacy NEXUSHEALTH_CARD_TOKEN: scan fallback in CardService).
- CAUTION (deployment): repo config points at NEW infra names - vercel.json destination + render.yaml APP_URL = https://pranacrux-backend.onrender.com, CORS_ORIGINS incl. https://pranacrux-eight.vercel.app, render.yaml repo = https://github.com/ganeswarikuramdasu/PranaCrux.git. USER'S PLAN (agreed 2026-09-16): user renames infra on web dashboards (Render service -> pranacrux-backend, Vercel project -> pranacrux-eight + fix Root Dir = repo root + redeploy, GitHub repo -> PranaCrux); agent already set local origin to PranaCrux.git. Until renamed, live services are still https://gprana-backend.onrender.com (backend, health UP, login 200) and https://g-prana-eight.vercel.app (Vercel 404s on all routes due to stale Root Directory setting).
