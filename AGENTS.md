...
- REBRAND G-Prana -> PranaCrux applied repo-wide (see Work State / 2026-09-16 REBRAND note above). Commits pending.
- Verified: backend mvn -q -DskipTests package -> ackend-java/target/pranacrux-backend.jar; frontend 
pm run lint (tsc --noEmit) + 
pm run build (vite) clean.
- CAUTION (deployment): repo now points at NEW infra names - vercel.json destination + render.yaml APP_URL = https://pranacrux-backend.onrender.com, CORS_ORIGINS incl. https://pranacrux-eight.vercel.app, render.yaml repo = https://github.com/ganeswarikuramdasu/PranaCrux.git. The CURRENTLY-LIVE services are still https://gprana-backend.onrender.com (backend, health UP, login 200) and https://g-prana-eight.vercel.app (Vercel frontend, currently 404s on all routes due to stale Root Directory setting - needs manual fix + redeploy on Vercel dashboard: Root Dir = repo root, Build = npm run build, Output = dist, Framework = Vite). Remote origin still = G-Prana.git. Rename Render service -> pranacrux-backend and GitHub repo -> PranaCrux, or flip config back to the live URLs.
