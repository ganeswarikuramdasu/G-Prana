...
/**
 * Email delivery for G-Prana OTP codes via the Brevo REST API
...
public class EmailService {
    ...
    @Value("${gprana.email.brevo-api-key:}")
    private String brevoApiKey;

    @Value("${gprana.email.from:G-Prana Identity <ganeswarikuramdasu@gmail.com>}")
    ...
    public boolean sendOtpEmail(String toEmail, String otpCode) {
        ...
        String body = """
            {
              "sender": { "name": "G-Prana", "email": "%s" },
              "to": [ { "email": "%s" } ],
              "subject": "G-Prana Verification Code: %s",
              "htmlContent": %s
            }
