 ...
 * removable:
 *
 *   - Only runs when gprana.demo.seed.enabled=true (default false).
 ...
 *   SUPER_ADMIN    ganeswarikuramdasu@gmail.com Admin@Gprana2026!
 */
@Component
public class DemoAccountsSeeder implements CommandLineRunner {
    ...
    @Value("${gprana.demo.seed.enabled:false}")
    ...
    public void run(String... args) {
        if (!seedEnabled) {
            log.info("[demo-seed] disabled (gprana.demo.seed.enabled=false) - no demo password changes");
            return;
        }
