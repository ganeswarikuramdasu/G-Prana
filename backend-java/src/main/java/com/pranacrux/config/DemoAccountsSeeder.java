 ...
 * removable:
 *
 *   - Only runs when pranacrux.demo.seed.enabled=true (default false).
 ...
 *   SUPER_ADMIN    ganeswarikuramdasu@gmail.com Admin@Pranacrux2026!
 */
@Component
public class DemoAccountsSeeder implements CommandLineRunner {
    ...
    @Value("${pranacrux.demo.seed.enabled:false}")
    ...
    public void run(String... args) {
        if (!seedEnabled) {
            log.info("[demo-seed] disabled (pranacrux.demo.seed.enabled=false) - no demo password changes");
            return;
        }
